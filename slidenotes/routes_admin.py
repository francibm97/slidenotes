...
def conversions_per_dates():
    conversions_dates = dict(admin_db.session.query(sqlalchemy.func.strftime("%Y-%m-%d", Conversion.timestamp_uploaded), sqlalchemy.func.count(sqlalchemy.func.strftime("%Y-%m-%d", Conversion.timestamp_uploaded))).filter(Conversion.timestamp_uploaded >= (datetime.datetime.today() - datetime.timedelta(days=56))).group_by(sqlalchemy.func.strftime("%Y-%m-%d", Conversion.timestamp_uploaded)).all())
    dates = list(reversed([datetime.datetime.today() - datetime.timedelta(days=days) for days in range(0,56)]))
    dates_last_weeks = [date.strftime("%Y-%m-%d") for date in dates[-28:]]
    dates_prev_weeks = [date.strftime("%Y-%m-%d") for date in dates[-56:-28]]
